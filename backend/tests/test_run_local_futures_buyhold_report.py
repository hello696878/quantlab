...
def test_es_buyhold_pnl_correct(tmp_path):
    ...
    result = _run(BUYHOLD_SCRIPT, "--input", str(tmp_path / "ES_daily.csv"))
    assert result.returncode == 0, result.stdout + result.stderr
...
def test_nq_buyhold_pnl_correct(tmp_path):
    _normalize_fixtures(tmp_path)
    result = _run(BUYHOLD_SCRIPT, "--input", str(tmp_path / "NQ_daily.csv"))
    assert result.returncode == 0, result.stdout + result.stderr
...
def test_tick_and_multiplier_pnl_match(tmp_path):
    ...
    _normalize_fixtures(tmp_path)
    result = _run(BUYHOLD_SCRIPT, "--input", str(tmp_path))
    assert result.returncode == 0, result.stdout + result.stderr
