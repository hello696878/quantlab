...
- Benchmark & active analytics: a v1 `benchmark` config on single-asset backtests and Strategy Comparison — **Buy & Hold Same Asset** (default; the engine's costless built-in benchmark), **Custom Benchmark** ticker (e.g. SPY/QQQ/BTC-USD, fetched via the same provider seam with its own data-quality diagnostics, **inner-joined by date**), or **None**. Responses add a `benchmark_analytics` block: benchmark total return / CAGR / volatility / Sharpe / max drawdown plus active metrics — **excess return, alpha, beta, correlation, tracking error, information ratio** (risk-free rate 0, annualized with the selected convention). Non-computable metrics (zero variance, zero tracking error, limited overlap) are **null with a warning**, never NaN. Benchmark choice **never changes strategy trades or results**, and the legacy same-asset benchmark fields/chart stay unchanged. *(Single-asset Backtest + Strategy Comparison; the per-strategy comparison table gains a "vs Benchmark" toggle. Pairs/CSV custom benchmarks and portfolio tools are future work.)*
- Pydantic v2 request/response schemas with full validation
- 1,000+ pytest tests (49 files) using synthetic data — no network calls at test time
