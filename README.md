...
- Position sizing: a v1 `position_sizing` on single-asset backtests (Backtest form → **Position sizing** selector) — **Full Allocation** (default, backward-compatible), **Fixed Fraction**, **Volatility Target** (no-lookahead rolling realized vol, capped at 100% — de-levers only), or **Max Exposure** (cash reserve). Sizing scales exposure magnitude only — signal timing/direction are unchanged and **no leverage** (`|exposure| ≤ 1`). The response reports `average_exposure` and echoes the resolved sizing. *(Applies to the single-asset directional strategies; pairs and the research tools use full allocation.)*
- Pydantic v2 request/response schemas with full validation
- 880+ pytest tests (45 files) using synthetic data — no network calls at test time
