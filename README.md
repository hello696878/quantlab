...
| **Global Markets Globe** | Interactive **3D globe** (pure-SVG orthographic projection · drag-to-rotate · auto-rotate · no WebGL/Three.js dependency) with **15 sample markets** · clickable pulsing markers + hover tooltips · region filter & search · keyboard-accessible market list · per-country **dossier** (equity indices + sparklines · macro snapshot · currency & rates · market structure · sample headlines with sentiment tags · QuantLab cross-links) — **static illustrative sample data**, not real-time quotes, live FRED macro / delayed index & FX quotes / news-sentiment / GeoJSON borders planned, not investment advice |
