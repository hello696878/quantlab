# QuantLab — Project Snapshot (Phase 48.0)

A one-page handoff doc. Facts verified against the repo when written
(version label `4.66.0-dev`); counts drift as phases land — re-verify before
public use.
...
- **Product Workflow:** Scenario Studio, Research Workspace, Experiment
  Registry, Data Reliability Center, QA Command Center, Release Notes Center,
...
the frozen demo path and the Experiment Registry view (local-first, plus a
manually triggered CI workflow — `CI_BROWSER_E2E.md`); **no frontend
