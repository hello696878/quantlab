# QuantLab — Project Snapshot (Phase 60.0)

A one-page handoff doc. Facts verified against the repo when written
(version label `4.78.0-dev`); counts drift as phases land — re-verify before
public use.
...
Diagnostics, Regime Diagnostics, Cost & Capacity, Portfolio Diagnostics, Portfolio Stress Lab, Portfolio Attribution, Factor Diagnostics, and Signal Decay Lab views (local-first, plus a manually triggered CI workflow —
