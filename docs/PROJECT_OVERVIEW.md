...
- **Release Notes Center** (`ReleaseNotesCenterPanel`, view `releasenotes`, 40.0) — a frontend-only static release-management reference (no backend endpoint, no git/GitHub access from the browser): current version card, release areas, changelog summaries, the release flow, and a copyable release-notes skeleton. Canonical sources: `VERSION`, `CHANGELOG.md`, `docs/VERSION_MANIFEST.md`, `docs/RELEASE_CHECKLIST.md`, `docs/RELEASE_NOTES_TEMPLATE.md`, `docs/MILESTONE_HISTORY.md`, `docs/PROJECT_SNAPSHOT.md`; the read-only live summary is `scripts\print_release_summary.ps1`.
