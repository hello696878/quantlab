...
**`4.74.0-dev`** (see the repo-root [`VERSION`](../VERSION) file). The next
expected tag on completion of the current phase's review is
`v4.74.0-portfolio-risk-budget-constraint-diagnostics-v1` (Phase 56) —
...
per feature phase). 119 local tags existed when this manifest was last
updated (Phase 56.0); the latest verified tag is
`v4.73.0-transaction-cost-slippage-capacity-diagnostics-v1` (the frozen
