...
**`4.66.0-dev`** (see the repo-root [`VERSION`](../VERSION) file). The next
expected tag on completion of the current phase's review is
`v4.66.0-research-experiment-registry-v1` — "expected" because
...
per feature phase). 112 local tags existed when this manifest was last
updated (Phase 48.0); the latest verified tag is
`v4.65.0-post-publication-verification-baseline-v1` (the frozen
