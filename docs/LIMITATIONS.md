...
**UI fix (16.1):** the Yield Curve Lab **Bond Pricing result-card value text** was hard to read — the dark-theme slate ramp is inverted (low shades are dark surfaces), so the value text was rendered in a near-invisible dark colour. Result-card values now use bright slate / a blue accent with adequate contrast; labels stay muted.
