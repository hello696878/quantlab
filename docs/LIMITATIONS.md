...
**The AFML Methodology Lab is a labeling, validation-splitting, and sampling toolkit, not a trained model.** It demonstrates the *pre-modeling pipeline* from *Advances in Financial Machine Learning* on a **synthetic price path** (clearly labelled, not live data): symmetric **CUSUM event sampling**, **triple-barrier labeling** (profit-take / stop-loss / vertical → +1 / −1 / 0), **sample concurrency**, **sample-uniqueness weights** (uniqueness = mean of 1/concurrency over each label's life, normalized to mean 1), **purged K-fold + embargo diagnostics**, and **sequential bootstrap**. CUSUM thresholds are explicit: **fixed** mode uses an absolute return threshold (for example 0.02 = 2%), while **vol-scaled** mode uses a multiplier of the rolling volatility estimate. It is **leakage-aware**: event *formation* uses only information available up to that bar, and future prices are used **only to assign labels** (as labeling fundamentally requires) — not to form events. Results depend on the event definition, barrier multiples, volatility estimate, threshold, embargo, sample construction, and data quality. There are no features, no fitted model, and no performance evaluation, so it produces **no predictions and no performance claims**. **Meta-labeling, information-driven bars, fractional differentiation, and Combinatorial Purged CV remain planned, not implemented.** It is **not** a full AFML implementation and not investment advice.

**Purged K-Fold + Embargo CV (19.1) reduces leakage but does not bless a model.** The Purged CV tab keeps the K-fold **time order** (no shuffle), then **purges** training labels whose inclusive `[start, end]` intervals overlap any label in the test fold and **embargoes** remaining training labels that start just after the latest test-label end (embargo applied after purging). `embargo_pct` is a fraction of the full synthetic observation history and is rounded up to whole trading observations. Per-fold diagnostics measure overlap immediately *before* and *after purging*; after-purge overlap must be zero. This addresses the specific leakage from overlapping financial labels — it does **not** guarantee a good model, eliminate other research bias (multiple testing, regime shifts, costs), or constitute model training. It is **not Combinatorial Purged CV (CPCV)** — CPCV, meta-labeling, and fractional differentiation remain planned. Synthetic demo data only; not investment advice.
...
**Fractional Differentiation (19.3) is preprocessing, not a signal, and its diagnostics are heuristic.** The Fractional Differentiation tab applies **fixed-width fractional differencing** to a synthetic price path: recursive weights (`w[0]=1`, `w[k] = -w[k-1]·(d-k+1)/k`) truncated once they fall below a threshold, then `fracdiff[t] = Σ_k w[k]·series[t-k]` after a warmup of `width-1` bars (earlier rows are dropped). It targets the trade-off in AFML chapter 5: ordinary first differencing makes a series stationary but **erases almost all memory**, whereas `0 < d < 1` removes enough non-stationarity to be usable while preserving more memory (a higher correlation with the level). Caveats: it operates on the **close price** (not log price) for clarity; `d`, the weight threshold, and proper validation are critical and are **not** auto-tuned; and the **stationarity-style diagnostics here are heuristic** (trend slope + lag-1 autocorrelation) — **not a formal stationarity test** (no ADF; statsmodels is intentionally not added). It is **preprocessing, not a trading signal**, has no model attached, and is not a full AFML implementation. Synthetic demo data; not investment advice.
