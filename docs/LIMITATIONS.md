...
**The AFML Methodology Lab is a labeling toolkit, not a trained model.** It demonstrates the *pre-modeling labeling pipeline* from *Advances in Financial Machine Learning* on a **synthetic price path** (clearly labelled, not live data): symmetric **CUSUM event sampling**, **triple-barrier labeling** (profit-take / stop-loss / vertical → +1 / −1 / 0), **sample concurrency**, and **sample-uniqueness weights** (uniqueness = mean of 1/concurrency over each label's life, normalized to mean 1). CUSUM thresholds are explicit: **fixed** mode uses an absolute return threshold (for example 0.02 = 2%), while **vol-scaled** mode uses a multiplier of the rolling volatility estimate. It is **leakage-aware**: event *formation* uses only information available up to that bar, and future prices are used **only to assign labels** (as labeling fundamentally requires) — not to form events. Results depend on the event definition, barrier multiples, the volatility estimate, the threshold, and data quality. This is **not a model**: there are no features, no fitted model, and no out-of-sample evaluation, so it produces **no predictions and no performance claims**. **Meta-labeling, information-driven bars, sequential bootstrap, fractional differentiation, and purged K-fold / Combinatorial Purged CV are planned, not implemented** — so the lab demonstrates *why* naive random train/test splits leak on overlapping financial labels but does **not** yet provide the leakage-free cross-validation that real model training needs. It is **not** a full AFML implementation and not investment advice.

**Purged K-Fold + Embargo CV (19.1) reduces leakage but does not bless a model.** The Purged CV tab keeps the K-fold **time order** (no shuffle), then **purges** training labels whose intervals overlap the test fold and **embargoes** training labels that start just after it (embargo applied after purging; intervals inclusive). Per-fold diagnostics show overlap *before* purging (can be > 0) and *after* (= 0). This addresses the specific leakage from overlapping financial labels — it does **not** guarantee a good model, eliminate other research bias (multiple testing, regime shifts, costs), or constitute model training. It is **not Combinatorial Purged CV (CPCV)** — CPCV, meta-labeling, sequential bootstrap, and fractional differentiation remain planned. Synthetic demo data only; not investment advice.
