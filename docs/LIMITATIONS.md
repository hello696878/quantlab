...
**The Global Markets Globe (20.0 v1 → 20.1 v1.1) is a static illustrative showcase, not a market data terminal.** The `/globe` workspace renders an interactive **dependency-free 3D globe** in a three-zone "mission-control" layout. v1.1 upgrades the renderer to a hand-built **canvas 2D** orthographic globe (dot-matrix landmass, atmosphere halo, starfield, 30° graticule, region-colored pulsing markers, illustrative great-circle "capital-flow" arcs, drag/auto-rotate/reset) — still **no Three.js and no WebGL**, with a graceful fallback message and the keyboard-accessible market list if a 2D canvas context is unavailable, and `prefers-reduced-motion` gating for the animations. It covers **15 hand-authored sample markets**, each with a financial dossier (Market Pulse indices + illustrative sparklines, Macro Vitals, FX & Rates, Market Structure, Sample Headlines with sentiment pills, an illustrative bias pill, and QuantLab cross-links). The arcs and the bias pill are **decorative/illustrative** — not capital-flow measurements, forecasts, or signals. **Data Layer (20.2):** the dossiers are now served by a typed backend API (`GET /globe/markets`, `/globe/markets/{id}`, `/globe/regions`) and the frontend falls back to the bundled dataset if the backend is unavailable (with a visible "bundled static fallback" warning) — but this is a **data-architecture step, not a live-data step**: the backend payload is still 100% static sample data (every record is `is_sample`, `source_status` is `static_sample`, and the UI surfaces index levels / FX as "Sample"). The delayed index / FX quote and news/sentiment adapters are **inert stubs that raise `NotImplementedError`** — they make no HTTP calls and fetch nothing. **FRED macro adapter (20.3):** an *optional, opt-in* macro integration exists but is **disabled by default** and requires no API key for normal local development. It only enriches **macro** fields (US only in v1: policy rate, unemployment, GDP growth; inflation and debt/GDP stay static), and **fails closed to static sample data** on disabled / missing-key / network-error / invalid-value, marking `source_status.macro` as `fred_unavailable` with a warning. The FRED API key is read from the environment and is **never** returned to the frontend or committed (`.env` is gitignored; see `docs/GLOBE_DATA.md`). Indices, FX, market structure, and news remain static sample in all configurations. No live data is claimed; not real-time, not investment advice. **Every value is static, deterministic, illustrative sample data** — index "levels" are the literal text "Sample", FX values are "Sample", daily change percentages and sparklines are illustrative shapes (a seeded walk), and macro figures (GDP growth, inflation, unemployment, policy rate, debt-to-GDP), market-cap, and listed-company figures are placeholders. **Nothing here is real-time, live, or fetched** — there is **no live market data, no delayed quotes, no live FX, no live news, and no web scraping**, and the app makes **no claim of real-time coverage, live country-level financial data, or complete country coverage**. Sample headlines and sentiment tags are illustrative, **not** a news feed or a sentiment model. Cross-links open the related QuantLab module but do **not** yet pre-fill market-specific context. The globe markers are placed by latitude/longitude only; there is **no GeoJSON country-border highlighting**. Planned future work: live **FRED** macro, delayed index / FX quotes, a news / sentiment feed, GeoJSON borders, and market-aware deep links. The market list below/beside the globe is the keyboard-accessible path and works without any 3D rendering. Educational/research only, **not investment advice**.
