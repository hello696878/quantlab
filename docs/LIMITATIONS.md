...
**The AFML Methodology Lab is a labeling and validation-splitting toolkit, not a trained model.** It demonstrates the *pre-modeling pipeline* from *Advances in Financial Machine Learning* on a **synthetic price path** (clearly labelled, not live data): symmetric **CUSUM event sampling**, **triple-barrier labeling** (profit-take / stop-loss / vertical → +1 / −1 / 0), **sample concurrency**, **sample-uniqueness weights** (uniqueness = mean of 1/concurrency over each label's life, normalized to mean 1), and **purged K-fold + embargo diagnostics**. CUSUM thresholds are explicit: **fixed** mode uses an absolute return threshold (for example 0.02 = 2%), while **vol-scaled** mode uses a multiplier of the rolling volatility estimate. It is **leakage-aware**: event *formation* uses only information available up to that bar, and future prices are used **only to assign labels** (as labeling fundamentally requires) — not to form events. Results depend on the event definition, barrier multiples, volatility estimate, threshold, embargo, and data quality. There are no features, no fitted model, and no performance evaluation, so it produces **no predictions and no performance claims**. **Meta-labeling, information-driven bars, sequential bootstrap, fractional differentiation, and Combinatorial Purged CV remain planned, not implemented.** It is **not** a full AFML implementation and not investment advice.
...
**Sequential Bootstrap (19.2) reduces sample dependence, not model risk.** The Sequential Bootstrap tab draws labeled events one at a time with probability **proportional to the marginal average uniqueness** they add (via an indicator matrix of when each label is active), producing a less-overlapping sample than a uniform random bootstrap. The summary's sequential-vs-random uniqueness are **Monte-Carlo means** (a fair comparison); the table/path show one representative draw. This **reduces the dependence** between samples used to fit a model — it does **not** guarantee a better model, more profit, or valid research, and the benefit is small when labels barely overlap (it grows with overlap, e.g. longer vertical barriers or more frequent events). Without replacement requires `sample_size ≤ events`. It is methodology only — no features, no model, no out-of-sample performance — and not a full AFML implementation. Synthetic demo data; not investment advice.
