...
/**
 * Shared neutral status pills for the Portfolio Attribution Lab (Phase 58.0),
 * following the ExperimentRegistryShared / PortfolioStressShared convention
 * so the panel and the detail view never import each other.
 */

import { INTEGRITY_LABELS } from "@/lib/portfolioAttribution";

export function IntegrityPill({ status }: { status: string }) {
  const styles: Record<string, string> = {
    verified_from_stored_rebalance: "border-emerald-200 bg-emerald-50 text-emerald-700",
    verified_causal_weights: "border-emerald-200 bg-emerald-50 text-emerald-700",
...
export function ReconciliationPill({ status }: { status: string }) {
  const styles: Record<string, string> = {
    reconciled: "border-emerald-200 bg-emerald-50 text-emerald-700",
    residual: "border-amber-200 bg-amber-50 text-amber-700",
    unknown: "border-slate-200 bg-slate-50 text-slate-500",
  };
  return (
    <span className={`inline-block rounded-full border px-2 py-0.5 text-[11px] font-medium ${styles[status] ?? styles.unknown}`}>
      {status}
    </span>
  );
}
