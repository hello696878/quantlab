...
import MetaLabelingDetail, { OofPill, StatusPill } from "@/components/MetaLabelingDetail";

type Mode = "list" | "detail" | "compare";
type Selected = { id: number; name: string };

const PAGE_SIZE = 15;
const METHODS = ["none", "sigmoid", "isotonic"];
const OOF_OPTIONS = ["verified_from_validation_run", "declared_out_of_fold", "not_out_of_fold", "unknown"];
const STATUSES = ["pending", "completed", "failed", "invalidated"];

const DISCLAIMER =
  "Local-first meta-labeling research lab: a secondary layer that asks whether the primary signal's direction was correct under a documented outcome rule, whether predicted probabilities are reliable (calibration fitted on training data only, out-of-fold via Model Validation splits where linked), and how decision thresholds trade coverage against precision and recall. Meta-label 1 means the research outcome condition was met — never that a trade is profitable. No best-model or best-threshold selection, no position sizing, no execution, not certification, not investment advice.";

export default function MetaLabelingPanel({ onNav }: { onNav?: (view: string) => void }) {
  ...
  async function handleSeed() {
    ...
    try {
      ...
      toast.success(
        "Demo loaded",
        res.created_runs > 0
          ? `${res.created_runs} runs and ${res.created_policies} threshold policies created.`
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url;
      a.download = `quantlab-meta-labeling-${new Date().toISOString().slice(0, 10)}.json`;
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs font-medium text-slate-500">Calibration integrity · SQLite · no cloud</span>
            </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
          ...
          <Card label="OOF verified" value={summary.oof_verified} />
          <Card label="Calibrated" value={summary.calibrated} />
          <Card label="Threshold policies" value={summary.threshold_policies} />
      ...
      <div className="card p-3">
        <div className="flex flex-wrap items-end gap-2">
          <Select label="Calibration" value={filters.calibration_method ?? ""} options={METHODS}
            onChange={(v) => updateFilter("calibration_method", v)} />
          <Select label="OOF status" value={filters.oof_status ?? ""} options={OOF_OPTIONS} labels={OOF_LABELS}
            onChange={(v) => updateFilter("oof_status", v)} />
          <Select label="Status" value={filters.status ?? ""} options={STATUSES}
            onChange={(v) => updateFilter("status", v)} />
          <div className="flex flex-col gap-1">
            <label htmlFor="meta-search" className="text-xs font-medium text-slate-500">Search</label>
            <input id="meta-search" type="text" value={filters.query ?? ""}
      ...
      {loading ? (
        <SkeletonTable rows={6} cols={8} caption="Loading meta-label runs…" />
      ) : error ? (
        ...
        <EmptyState
          title="No meta-label runs yet"
          description="Load the deterministic demo to explore calibration, out-of-fold integrity, and threshold trade-offs — or create runs via the API."
            ...
            <table className="w-full min-w-[1200px] text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  <th scope="col" className="px-3 py-3 text-left">Calibration</th>
                  <th scope="col" className="px-3 py-3 text-left">OOF</th>
                  ...
                  <th scope="col" className="px-3 py-3 text-right">Prevalence</th>
                  <th scope="col" className="px-3 py-3 text-right">Brier</th>
                  <th scope="col" className="px-3 py-3 text-right">ECE</th>
              ...
              <tbody>
                {list.items.map((row) => (
                  <tr key={row.id} className="border-b border-slate-50 transition-colors hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3 text-xs text-slate-600">{row.calibration_method}</td>
                    <td className="px-3 py-3"><OofPill status={row.oof_status} /></td>
                    ...
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.positive_prevalence ?? "—"}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.brier_preview ?? "—"}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.ece_preview ?? "—"}</td>
...
function Select({ label, value, options, labels, onChange }: {
  ...
  const id = `meta-filter-${label.toLowerCase().replace(/\s+/g, "-")}`;
  return (
    <div className="flex flex-col gap-1">
...
// ---------------------------------------------------------------------------
// Compare view (neutral grouped diff)
// ---------------------------------------------------------------------------

const GROUP_LABELS: Record<string, string> = {
  identity: "Identity",
  label_policy: "Label policy",
  probability_metrics: "Probability metrics",
};

function MetaCompare({ pair, onBack }: { pair: { a: Selected; b: Selected }; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4">
        <h2 className="text-lg font-bold text-slate-900">Compare meta-label runs</h2>
        ...
        <p className="mt-2 text-xs text-slate-400">
          Differences are reported neutrally — neither run is labelled better, profitable, or recommended.
        </p>
      </div>
      {error ? (
