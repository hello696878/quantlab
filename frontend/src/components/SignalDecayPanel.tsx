...
import SignalDecayDetail from "@/components/SignalDecayDetail";
import {
  CompletenessPill,
  IntegrityPill,
  OverlapPill,
...
const OVERLAP_OPTIONS = ["non_overlapping", "partially_overlapping",
                         "overlapping"];
const SIGNAL_TYPE_OPTIONS = ["continuous_score", "probability", "rank",
                             "ordinal_category", "binary_indicator",
                             "user_supplied_descriptive"];

const DISCLAIMER =
  "Local-first signal-decay diagnostics: descriptive associations between STORED signal observations and later outcomes across EXPLICIT forecast horizons and implementation delays. Signal availability timing is enforced (an outcome that begins before its signal was available marks the run invalid), horizon overlap is measured and disclosed rather than hidden, ties, constants and small samples are conservatively unavailable, every p-value is a real statistical value with its overlap limitation attached, and the equal-weight bucket reference keeps gross and cost-adjusted results separate. Nothing here proves predictability or alpha, guarantees that a relationship persists, recommends a horizon, lag, threshold, signal or trade, selects a strategy, sizes a position, monitors or executes anything, or constitutes investment advice.";

export default function SignalDecayPanel({ onNav }: { onNav?: (view: string) => void }) {
  ...
  async function handleSeed() {
    ...
    try {
      ...
      toast.success(
        "Demo loaded",
        res.created_count > 0
          ? `${res.created_count} signal-decay runs created.`
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url;
      a.download = `quantlab-signal-decay-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  if (mode === "detail" && detail) {
    return (
      <SignalDecayDetail
        ...
        onOpenFactor={onNav ? () => onNav("factordiagnostics") : undefined}
        onOpenMetaLabel={onNav ? () => onNav("metalabeling") : undefined}
      />
    );
  }
  ...
  return (
    <div className="space-y-4" data-testid="signal-decay-panel">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs font-medium text-slate-500">
                Explicit horizons &amp; lags · overlap disclosed · no predictability claims · SQLite
              </span>
            </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-7" data-testid="signal-summary-cards">
          <Card label="Runs" value={summary.runs} />
          <Card label="Completed" value={summary.completed} />
          <Card label="Signals" value={summary.signals} />
          <Card label="Observations" value={summary.observations} />
          <Card label="Horizon rows" value={summary.horizon_rows} />
          <Card label="Overlapping runs" value={summary.overlapping_runs} />
      ...
      <div className="card p-3">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <Select label="Overlap" value={filters.overlap_status ?? ""}
            options={OVERLAP_OPTIONS} labels={OVERLAP_LABELS}
            onChange={(v) => updateFilter("overlap_status", v)} />
          <Select label="Signal type" value={filters.signal_type ?? ""}
            options={SIGNAL_TYPE_OPTIONS} labels={SIGNAL_TYPE_LABELS}
            onChange={(v) => updateFilter("signal_type", v)} />
          <div className="flex flex-col gap-1">
            <label htmlFor="sdx-search" className="text-xs font-medium text-slate-500">Search</label>
            <input id="sdx-search" type="text" value={filters.query ?? ""}
              onChange={(e) => updateFilter("query", e.target.value)}
              placeholder="name, description or signal" className="ql-input w-52 px-2 py-1.5 text-sm" />
          </div>
      ...
      {loading ? (
        <SkeletonTable rows={6} cols={8} caption="Loading signal-decay runs…" />
      ) : error ? (
        ...
        <EmptyState
          title="No signal-decay runs yet"
          description="Load the deterministic demo to explore hand-computable perfect and decaying relationships, ties and constants, cross-sectional rank IC, overlapping versus non-overlapping horizons, monotone and U-shaped buckets, turnover and holding cohorts, implementation delays, cost-adjusted references, stored-regime and held-out validation views — or create runs via the API."
          actions={[{ label: seeding ? "Loading…" : "Load demo runs", onClick: handleSeed }]}
        />
      ) : (
        <div className="card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1500px] text-sm" data-testid="signal-runs-table">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  <th scope="col" className="px-3 py-3 text-left">Signal type</th>
                  <th scope="col" className="px-3 py-3 text-left">Outcome</th>
                  <th scope="col" className="px-3 py-3 text-right">Entities</th>
                  <th scope="col" className="px-3 py-3 text-right">Obs.</th>
                  <th scope="col" className="px-3 py-3 text-right">Horizons</th>
                  <th scope="col" className="px-3 py-3 text-right">Rank IC (h1)</th>
                  <th scope="col" className="px-3 py-3 text-right">Turnover (1-way)</th>
                  <th scope="col" className="px-3 py-3 text-left">Overlap</th>
              ...
              <tbody>
                {list.items.map((row) => (
                  <tr key={row.id} className="border-b border-slate-50 transition-colors hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3 text-xs">{SIGNAL_TYPE_LABELS[row.signal_type] ?? row.signal_type}</td>
                    <td className="px-3 py-3 text-xs">{row.outcome_target_type.replace(/_/g, " ")}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.entity_count}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.observation_count}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.horizon_count}×{row.lag_count}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{fmtNum(row.first_horizon_rank_ic)}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{fmtPct(row.mean_one_way_turnover, 1)}</td>
                    <td className="px-3 py-3"><OverlapPill status={row.overlap_status} /></td>
...
function Select({ label, value, options, labels, onChange }: {
  ...
  const id = `sdx-filter-${label.toLowerCase().replace(/\s+/g, "-")}`;
  return (
    <div className="flex flex-col gap-1">
...
function SignalDecayCompare({ pair, onBack }: { pair: { a: Selected; b: Selected }; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4" data-testid="signal-compare">
      <button type="button" onClick={onBack}
        className="rounded-md border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-50">
        ← Back to runs
      </button>
      <div className="card p-4">
        <h2 className="text-lg font-bold text-slate-900">Compare signal-decay runs</h2>
        ...
        {data && (
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            {Object.entries(data.fields).map(([name, state]) => (
              <span key={name} className={`rounded-full border px-2 py-0.5 font-medium ${
                state === "same"
                ...
                {name.replace(/_/g, " ")}: {state.replace(/_/g, " ")}
              </span>
            ))}
          </div>
        )}
        <p className="mt-2 text-xs text-slate-400">
          {data?.note ?? "Differences are reported neutrally — no run is better and no winner is declared."}
        </p>
      </div>
      {error ? (
        ...
        <div className="card overflow-hidden">
          <div className="border-b border-slate-100 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700">
            Horizon rows ({data.horizon_rows.length})
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  <th scope="col" className="px-4 py-2 text-left">Horizon</th>
                  <th scope="col" className="px-4 py-2 text-right">Lag</th>
                  <th scope="col" className="px-4 py-2 text-right">A Spearman</th>
                  <th scope="col" className="px-4 py-2 text-right">B Spearman</th>
                  <th scope="col" className="px-4 py-2 text-right">A spread</th>
                  <th scope="col" className="px-4 py-2 text-right">B spread</th>
                  <th scope="col" className="px-4 py-2 text-left">Presence</th>
                </tr>
              </thead>
              <tbody>
                {data.horizon_rows.map((r, i) => (
                  <tr key={i} className="border-b border-slate-50 font-mono text-xs last:border-0">
                    <td className="px-4 py-1.5 font-sans">{r.horizon}</td>
                    <td className="px-4 py-1.5 text-right">{r.entry_lag}</td>
                    <td className="px-4 py-1.5 text-right">{fmtNum(r.a_spearman)}</td>
                    <td className="px-4 py-1.5 text-right">{fmtNum(r.b_spearman)}</td>
                    <td className="px-4 py-1.5 text-right">{fmtPct(r.a_spread)}</td>
                    <td className="px-4 py-1.5 text-right">{fmtPct(r.b_spread)}</td>
                    <td className="px-4 py-1.5 font-sans">{r.presence.replace(/_/g, " ")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
