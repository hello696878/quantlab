...
import FeatureDiagnosticsDetail, {
  IntegrityPill,
  StatusPill,
} from "@/components/FeatureDiagnosticsDetail";

type Mode = "list" | "detail" | "compare";
type Selected = { id: number; name: string };

const PAGE_SIZE = 15;
const METHODS = ["permutation", "native_impurity", "coefficient"];
const INTEGRITY_OPTIONS = ["verified_held_out", "declared_held_out", "not_held_out", "unknown"];
const STATUSES = ["pending", "completed", "failed", "invalidated"];
const METRICS = ["log_loss", "brier", "roc_auc", "pr_auc", "balanced_accuracy", "f1", "mae", "mse", "rmse", "r2"];

const DISCLAIMER =
  "Local-first feature-diagnostics research lab: held-out permutation importance (the model is fitted per validation split on training members only, and one feature at a time is permuted within the held-out samples), rank stability across folds, correlated-feature groups, and feature distribution / importance drift. Importance is a measured sensitivity under one method and one metric — it does not prove causality or profitability, no feature is selected or deleted automatically, no model is chosen for you, and nothing here is investment advice.";

export default function FeatureDiagnosticsPanel({ onNav }: { onNav?: (view: string) => void }) {
  ...
  async function handleSeed() {
    ...
    try {
      ...
      toast.success(
        "Demo loaded",
        res.created_runs > 0
          ? `${res.created_runs} feature-diagnostics runs created.`
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url;
      a.download = `quantlab-feature-diagnostics-${new Date().toISOString().slice(0, 10)}.json`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      toast.success("Export ready", `${(payload.runs as unknown[]).length} run(s) exported as JSON (no raw samples).`);
    } catch (err) {
      const cls = classifyApiError(err);
      if (cls.backendUnavailable) notifyBackendOffline();
  ...
  if (mode === "detail" && detail) {
    return (
      <FeatureDiagnosticsDetail
        ...
        onOpenMetaLabeling={onNav ? () => onNav("metalabeling") : undefined}
      />
    );
  }
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs font-medium text-slate-500">Held-out evaluation · SQLite · no cloud</span>
            </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
          ...
          <Card label="Held-out verified" value={summary.held_out_verified} />
          <Card label="Stable features" value={summary.stable_features} />
          <Card label="Drift flags" value={summary.drift_flags} />
      ...
      <div className="card p-3">
        <div className="flex flex-wrap items-end gap-2">
          <Select label="Method" value={filters.method ?? ""} options={METHODS}
            onChange={(v) => updateFilter("method", v)} />
          <Select label="Integrity" value={filters.integrity_status ?? ""} options={INTEGRITY_OPTIONS} labels={INTEGRITY_LABELS}
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="fd-search" className="text-xs font-medium text-slate-500">Search</label>
            <input id="fd-search" type="text" value={filters.query ?? ""}
      ...
      {loading ? (
        <SkeletonTable rows={6} cols={8} caption="Loading feature-diagnostics runs…" />
      ) : error ? (
        ...
        <EmptyState
          title="No feature-diagnostics runs yet"
          description="Load the deterministic demo to explore held-out permutation importance, rank stability, correlated groups and drift — or create runs via the API."
            ...
            <table className="w-full min-w-[1240px] text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  <th scope="col" className="px-3 py-3 text-left">Validation</th>
                  <th scope="col" className="px-3 py-3 text-right">Features</th>
                  <th scope="col" className="px-3 py-3 text-right">Splits</th>
                  <th scope="col" className="px-3 py-3 text-left">Rank 1</th>
                  <th scope="col" className="px-3 py-3 text-right">Stable / drift</th>
              ...
              <tbody>
                {list.items.map((row) => (
                  <tr key={row.id} className="border-b border-slate-50 transition-colors hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3 text-xs text-slate-600">{row.method}</td>
                    ...
                    <td className="max-w-[140px] truncate px-3 py-3 text-xs text-slate-600" title={row.dataset_name ?? undefined}>
                      {row.dataset_name ? `${row.dataset_name} · ${row.dataset_version_label}` : "—"}
                      {row.dataset_invalidated && <span className="ml-1 text-red-600">⚠</span>}
                    </td>
                    <td className="px-3 py-3 text-xs text-slate-600">{row.validation_method ?? "—"}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.feature_count}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.valid_split_count}/{row.split_count}</td>
                    <td className="max-w-[120px] truncate px-3 py-3 font-mono text-xs" title={row.top_feature ?? undefined}>
                      {row.top_feature ?? "—"}
                    </td>
                    <td className="px-3 py-3 text-right font-mono text-xs">
                      {row.stable_feature_count} / {row.drift_flag_count > 0
                        ? <span className="text-amber-600">{row.drift_flag_count}⚠</span>
                        : "0"}
                    </td>
...
function Select({ label, value, options, labels, onChange }: {
  ...
  const id = `fd-filter-${label.toLowerCase().replace(/\s+/g, "-")}`;
  return (
    <div className="flex flex-col gap-1">
...
// ---------------------------------------------------------------------------
// Compare view (neutral grouped diff + per-feature table)
// ---------------------------------------------------------------------------
...
function FeatureCompare({ pair, onBack }: { pair: { a: Selected; b: Selected }; onBack: () => void }) {
  ...
  const num = (v: number | null | undefined, digits = 6) =>
    v === null || v === undefined ? "—" : Number.isInteger(v) ? String(v) : v.toFixed(digits).replace(/0+$/, "").replace(/\.$/, "");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4">
        <h2 className="text-lg font-bold text-slate-900">Compare feature-diagnostics runs</h2>
        ...
        <p className="mt-2 text-xs text-slate-400">
          Differences are reported neutrally — neither run is labelled better, causal, profitable, or recommended.
        </p>
      </div>
      {error ? (
          ...
          {data.features.length > 0 && (
            <div className="card overflow-hidden">
              <div className="border-b border-slate-100 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700">
                Per-feature comparison ({data.features.length} features)
              </div>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[900px] text-xs">
                  <thead>
                    <tr className="border-b border-slate-100 text-left font-semibold uppercase tracking-wide text-slate-500">
                      ...
                      <th scope="col" className="px-3 py-2 text-right">A importance</th>
                      <th scope="col" className="px-3 py-2 text-right">B importance</th>
                      <th scope="col" className="px-3 py-2 text-right">|Δ|</th>
                      <th scope="col" className="px-3 py-2 text-right">Δ%</th>
                      <th scope="col" className="px-3 py-2 text-right">A rank</th>
                      <th scope="col" className="px-3 py-2 text-right">B rank</th>
                      <th scope="col" className="px-3 py-2 text-right">Rank change</th>
                      <th scope="col" className="px-3 py-2">Sign changed</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.features.map((f) => (
                      <tr key={f.feature_name} className="border-b border-slate-50 font-mono last:border-0">
                        ...
                        <td className="px-3 py-1.5">{f.availability.replace(/_/g, " ")}</td>
                        <td className="px-3 py-1.5 text-right">{num(f.a_importance)}</td>
                        <td className="px-3 py-1.5 text-right">{num(f.b_importance)}</td>
                        <td className="px-3 py-1.5 text-right">{num(f.abs_difference)}</td>
                        <td className="px-3 py-1.5 text-right">{f.pct_difference === null ? "—" : `${num(f.pct_difference, 2)}%`}</td>
                        <td className="px-3 py-1.5 text-right">{num(f.a_rank)}</td>
                        <td className="px-3 py-1.5 text-right">{num(f.b_rank)}</td>
                        <td className="px-3 py-1.5 text-right">{num(f.rank_change)}</td>
                        <td className="px-3 py-1.5">{f.sign_changed === null ? "—" : f.sign_changed ? "yes" : "no"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
