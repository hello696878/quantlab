...
import CostDiagnosticsDetail, { CompletenessPill, IntegrityPill, StatusPill } from "@/components/CostDiagnosticsDetail";
...
const INTEGRITY_OPTIONS = ["supplied_realized", "verified_causal_input",
                           "verified_from_dataset_lineage", "declared",
                           "full_sample_descriptive", "unknown", "invalid"];
const COMPLETENESS_OPTIONS = ["complete", "partial", "gross_only", "invalid"];
const OBSERVATION_TYPES = ["trade", "period"];

const DISCLAIMER =
  "Local-first execution-cost diagnostics: explicitly configured commission, spread, slippage and square-root market-impact assumptions are applied to supplied historical trade or period observations, with unit-safe conversions, a strict no-look-ahead policy for liquidity inputs (trailing windows only, lag ≥ 1, centered windows rejected), transparent gross-to-net reconciliation where missing inputs stay unavailable — never zero — plus break-even, sensitivity and capacity-scaling estimates with participation warnings. Nothing here executes orders, predicts real fills, guarantees market capacity, recommends trade sizes or brokers, proves profitability, or provides investment or execution advice.";

export default function CostDiagnosticsPanel({ onNav }: { onNav?: (view: string) => void }) {
  ...
  async function handleSeed() {
    ...
    try {
      ...
      toast.success(
        "Demo loaded",
        res.created_runs > 0
          ? `${res.created_runs} cost-diagnostic runs created.`
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url;
      a.download = `quantlab-cost-diagnostics-${new Date().toISOString().slice(0, 10)}.json`;
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs font-medium text-slate-500">Explicit assumptions · no order execution · SQLite · no cloud</span>
            </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
          ...
          <Card label="Participation warnings" value={summary.participation_warnings} />
          <Card label="Incomplete-input obs." value={summary.incomplete_input_observations} />
      ...
      <div className="card p-3">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <Select label="Completeness" value={filters.completeness_status ?? ""}
            options={COMPLETENESS_OPTIONS} labels={COMPLETENESS_LABELS}
            onChange={(v) => updateFilter("completeness_status", v)} />
          <Select label="Type" value={filters.observation_type ?? ""}
            options={OBSERVATION_TYPES}
            onChange={(v) => updateFilter("observation_type", v)} />
          <div className="flex flex-col gap-1">
            <label htmlFor="cd-search" className="text-xs font-medium text-slate-500">Search</label>
            <input id="cd-search" type="text" value={filters.query ?? ""}
      ...
      {loading ? (
        <SkeletonTable rows={6} cols={8} caption="Loading cost-diagnostic runs…" />
      ) : error ? (
        ...
        <EmptyState
          title="No cost-diagnostic runs yet"
          description="Load the deterministic demo to explore gross-to-net reconciliation, break-even, sensitivity and capacity diagnostics under explicit cost assumptions — or create runs via the API."
            ...
            <table className="w-full min-w-[1280px] text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  <th scope="col" className="px-3 py-3 text-left">Type</th>
                  ...
                  <th scope="col" className="px-3 py-3 text-right">Total cost</th>
                  <th scope="col" className="px-3 py-3 text-right">Net</th>
                  <th scope="col" className="px-3 py-3 text-right">Part. ⚠</th>
              ...
              <tbody>
                {list.items.map((row) => (
                  <tr key={row.id} className="border-b border-slate-50 transition-colors hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3 text-xs">{row.observation_type}</td>
                    ...
                    <td className="px-3 py-3 text-right font-mono text-xs">{totalCell(row.gross_total, row)}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{totalCell(row.total_cost, row)}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{totalCell(row.net_total, row)}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">
                      {row.participation_warning_count > 0
                        ? <span className="text-amber-600">{row.participation_warning_count}</span>
                        : "0"}
                    </td>
...
function totalCell(v: number | null, row: RunSummary): string {
  if (v === null) return "—";
  if (row.observation_type === "period") return `${(v * 100).toFixed(3)}%`;
...
function Select({ label, value, options, labels, onChange }: {
  ...
  const id = `cd-filter-${label.toLowerCase().replace(/\s+/g, "-")}`;
  return (
    <div className="flex flex-col gap-1">
...
function CostCompare({ pair, onBack }: { pair: { a: Selected; b: Selected }; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4">
        <h2 className="text-lg font-bold text-slate-900">Compare cost-diagnostic runs</h2>
        ...
        <p className="mt-2 text-xs text-slate-400">
          Differences are reported neutrally — neither run is labelled cheaper, better, or recommended.
        </p>
      </div>
      {error ? (
