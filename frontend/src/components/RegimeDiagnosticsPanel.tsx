...
import RegimeDiagnosticsDetail, { IntegrityPill, StatusPill } from "@/components/RegimeDiagnosticsDetail";
...
const INTEGRITY_OPTIONS = ["verified_from_validation_split", "verified_causal_rule",
                           "declared", "full_sample_descriptive", "unknown", "invalid"];

const DISCLAIMER =
  "Local-first regime diagnostics: candidate outcomes are conditioned on explicitly defined market regimes (volatility, trend, liquidity, drawdown state, user-supplied categories, bounded combinations) formed with a strict no-look-ahead policy — trailing windows only, effective labels lagged at least one period, threshold-fitting subsets carrying distinct integrity states, and full-sample fits always flagged as descriptive. Conditional statistics, coverage, rank stability, concentration and transition differences are measured observations under this configuration — regimes are never predictions, nothing here proves causality or profitability, no strategy is switched or selected, and nothing is investment advice.";

export default function RegimeDiagnosticsPanel({ onNav }: { onNav?: (view: string) => void }) {
  ...
  async function handleSeed() {
    ...
    try {
      ...
      toast.success(
        "Demo loaded",
        res.created_runs > 0
          ? `${res.created_runs} regime-diagnostic runs created.`
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url;
      a.download = `quantlab-regime-diagnostics-${new Date().toISOString().slice(0, 10)}.json`;
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs font-medium text-slate-500">No-look-ahead regimes · SQLite · no cloud</span>
            </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
          ...
          <Card label="Observed regimes" value={summary.observed_regimes} />
          <Card label="Verified definitions" value={summary.verified_definitions} />
          <Card label="Low-coverage warnings" value={summary.low_coverage_warnings} />
      ...
      <div className="card p-3">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="rd-search" className="text-xs font-medium text-slate-500">Search</label>
            <input id="rd-search" type="text" value={filters.query ?? ""}
      ...
      {loading ? (
        <SkeletonTable rows={6} cols={8} caption="Loading regime-diagnostic runs…" />
      ) : error ? (
        ...
        <EmptyState
          title="No regime-diagnostic runs yet"
          description="Load the deterministic demo to explore no-look-ahead regime labelling, conditional performance, rank stability and transitions — or create runs via the API."
            ...
            <table className="w-full min-w-[1240px] text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  <th scope="col" className="px-3 py-3 text-right">Definitions</th>
                  <th scope="col" className="px-3 py-3 text-right">Regimes</th>
                  <th scope="col" className="px-3 py-3 text-left">Integrity</th>
                  <th scope="col" className="px-3 py-3 text-right">Invalid defs</th>
                  <th scope="col" className="px-3 py-3 text-right">Low-cov ⚠</th>
              ...
              <tbody>
                {list.items.map((row) => (
                  <tr key={row.id} className="border-b border-slate-50 transition-colors hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.regime_definition_count}</td>
                    <td className="px-3 py-3 text-right font-mono text-xs">{row.observed_regime_count}</td>
                    <td className="px-3 py-3"><IntegrityPill status={row.integrity_status} /></td>
                    <td className="px-3 py-3 text-right font-mono text-xs">
                      {row.invalid_definition_count > 0
                        ? <span className="text-red-600">{row.invalid_definition_count}</span>
                        : "0"}
                    </td>
                    <td className="px-3 py-3 text-right font-mono text-xs">
                      {row.low_coverage_warning_count > 0
                        ? <span className="text-amber-600">{row.low_coverage_warning_count}</span>
                        : "0"}
                    </td>
...
function Select({ label, value, options, labels, onChange }: {
  ...
  const id = `rd-filter-${label.toLowerCase().replace(/\s+/g, "-")}`;
  return (
    <div className="flex flex-col gap-1">
...
function RegimeCompare({ pair, onBack }: { pair: { a: Selected; b: Selected }; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-4">
        <h2 className="text-lg font-bold text-slate-900">Compare regime-diagnostic runs</h2>
                ...
                <table className="w-full min-w-[720px] text-xs">
                  <thead>
                    <tr className="border-b border-slate-100 text-left font-semibold uppercase tracking-wide text-slate-500">
                      ...
                      <th scope="col" className="px-3 py-2">A classification</th>
                      <th scope="col" className="px-3 py-2">B classification</th>
                      <th scope="col" className="px-3 py-2 text-right">A sign consistency</th>
                      <th scope="col" className="px-3 py-2 text-right">B sign consistency</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.robustness.map((r) => (
                      <tr key={`${r.candidate_id}|${r.definition_id}`}
                        ...
                        <td className="px-3 py-1.5">{r.candidate_id}</td>
                        <td className="px-3 py-1.5">{r.definition_id}</td>
                        <td className="px-3 py-1.5">{r.availability.replace(/_/g, " ")}</td>
                        <td className="px-3 py-1.5">{r.a_classification ?? "—"}</td>
                        <td className="px-3 py-1.5">{r.b_classification ?? "—"}</td>
                        <td className="px-3 py-1.5 text-right">{num(r.a_sign_consistency)}</td>
                        <td className="px-3 py-1.5 text-right">{num(r.b_sign_consistency)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
