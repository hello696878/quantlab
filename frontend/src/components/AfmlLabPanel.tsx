      ...
      {tab === "education" && (
        <div className="card space-y-3 p-5 text-sm text-slate-400">
          ...
          <ul className="space-y-2">
            <li><span className="font-semibold text-slate-900">Random train/test splits leak:</span> financial labels look forward in time and overlap, so a naive shuffle puts near-duplicate, time-adjacent samples in both train and test — inflating measured skill. A plain split does not fix this.</li>
            <li><span className="font-semibold text-slate-900">Purged K-fold (built):</span> the Purged CV tab keeps the K-fold time order but <em>purges</em> training labels whose intervals overlap the test fold, so train and test no longer share outcomes — the per-fold "overlap after purge" should be 0.</li>
            <li><span className="font-semibold text-slate-900">Embargo (built):</span> beyond purging, an embargo removes training labels that start just after the test fold, where serial correlation still leaks. It is applied after purging.</li>
            <li><span className="font-semibold text-slate-900">Sequential bootstrap (built):</span> a uniform bootstrap repeatedly samples overlapping (dependent) labels; the Sequential Bootstrap tab draws events with probability proportional to the marginal uniqueness they add, yielding a less-overlapping sample. It reduces dependence but does not guarantee better model performance — the benefit grows with how much the labels overlap.</li>
            <li><span className="font-semibold text-slate-900">Fractional differentiation (built):</span> ordinary first differencing can reduce persistence but also erase level memory; fractional differencing with 0 &lt; d &lt; 1 targets a trade-off between the two. Fixed width truncates tiny weights so the window does not grow without bound. It is preprocessing, not a signal, and these diagnostics are heuristic — not a formal stationarity test.</li>
            <li><span className="font-semibold text-slate-900">Event sampling (CUSUM):</span> sampling on a fixed clock oversamples quiet periods. CUSUM samples only when cumulative movement is large, focusing labels on informative moments. Fixed mode uses a percentage-return threshold; vol-scaled mode uses a rolling-volatility multiplier.</li>
            <li><span className="font-semibold text-slate-900">Triple-barrier labeling:</span> a label is set by which barrier (profit-take / stop-loss / vertical-time) is hit first — a path-dependent, risk-aware label, unlike a fixed-horizon return sign.</li>
            <li><span className="font-semibold text-slate-900">Timing:</span> events are sampled using information available up to the event date. Future prices are used only to assign supervised-learning labels, not as trade signals or recommendations.</li>
            <li><span className="font-semibold text-slate-900">Overlapping labels → dependence:</span> labels whose holding windows overlap share outcomes and are not independent. Concurrency counts the overlap at each bar.</li>
            <li><span className="font-semibold text-slate-900">Sample uniqueness:</span> each label's uniqueness is the average of 1/concurrency over its life; using these as sample weights stops crowded periods from dominating training.</li>
            <li><span className="font-semibold text-slate-900">Not a model:</span> this is labeling, validation splitting, overlap-aware sampling, and preprocessing only — no fitted model, synthetic data. Purged K-fold, embargo, sequential bootstrap, and fractional differentiation address specific methodology problems but do not guarantee a useful model. Meta-labeling, information-driven bars, and CPCV are planned. Not investment advice.</li>
          </ul>
