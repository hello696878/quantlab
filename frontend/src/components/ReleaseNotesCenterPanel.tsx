...
const VERSION_LABEL = "4.79.0-dev";
const EXPECTED_NEXT_TAG = "v4.79.0-signal-ensemble-redundancy-combination-diagnostics-v1";
const LATEST_VERIFIED_TAG = "v4.78.0-signal-decay-horizon-turnover-implementation-diagnostics-v1";
...
const CHANGELOG_CARDS: { tag: string; title: string; text: string }[] = [
  { tag: "v4.79 (next)", title: "Signal ensemble & redundancy lab", text: "Descriptive similarity, redundancy and EXPLICIT user-configured combination references over multiple stored signals: exact (entity, timestamp) alignment with missingness disclosed and never repaired, strict-intersection versus pairwise-complete policies with per-pair sample counts, real scipy pairwise statistics with constants, ties and thin overlap honestly unavailable, bucket and tail agreement, a correlation-distance matrix whose eigenvalue concentration and effective signal count are always labelled matrix concentration (never the true number of independent signals, with non-PSD matrices refused rather than repaired), optional scipy hierarchical clustering at an explicit threshold, four explicit combination modes whose per-observation contributions reconcile to 1e-9, require-all or opt-in renormalise missing-component policies, side-by-side component and combination evaluation through the Signal Decay policies with signed-book turnover (combining shown both cancelling and creating churn) and pinned notional-proportional costs, neutral leave-one-signal-out deltas, stored-regime, train/held-out and factor-residual views, multiple-testing adjustment beside raw p-values and seeded whole-cross-section bootstrap quantiles. Descriptions of stored samples — never proof of independence, diversification, predictability or alpha, never a signal, weight or ensemble recommendation, never an optimiser, and no market data is ever downloaded." },
  { tag: "v4.78", title: "Signal decay & implementation lag lab", text: "Descriptive association between stored signals and later outcomes across explicitly declared forecast horizons and implementation lags: exact-timestamp forward returns with per-pair availability enforcement (one violation makes the run invalid), a seven-state integrity axis separate from a three-state overlap axis, real scipy p-values with overlap limitations disclosed rather than corrected, per-timestamp cross-sectional rank IC, equal-count rank buckets with a neutral top-minus-bottom reference spread, sign-change / threshold / guarded-half-life decay summaries, one-way turnover with membership churn and holding-cohort overlap, notional-proportional cost adjustment from a pinned stored cost model with gross always separate, stored-regime, frozen-threshold held-out and factor-residual views, seeded bootstrap quantiles and multiple-testing adjustment shown beside raw values. Descriptions of stored samples — never proof of predictability or alpha, never a signal, horizon, lag or threshold recommendation, never a persistence guarantee, and no market data is ever downloaded." },
  { tag: "v4.77", title: "Factor exposure & macro sensitivity lab", text: "Measured sensitivity of one declared return series to SUPPLIED factor and macro observations: explicit categories, units, transformation formulas, lags and availability rules; exact timestamp alignment with nothing resampled, forward-filled or zero-filled; verified causal timing versus descriptive contemporaneous association, with a future-looking alignment available only as a declared-invalid state; vintage policies that keep a later revision out of an earlier fit; SVD least squares with honest rank, condition number and statistics withheld rather than infinite; an explicit ridge reference with no p-values; contribution reconciliation against the estimator's own residual; trailing rolling exposures a later observation cannot rewrite; benchmark-relative, stored-regime, stored-stress, attribution and held-out validation views. Measurements under stated assumptions — never proof of causality or alpha, never a return prediction, never a factor, macro or portfolio recommendation, and no market or macroeconomic data is ever downloaded." },
  { tag: "v4.76", title: "Portfolio attribution lab", text: "Measured performance attribution of stored portfolio weights: beginning-of-period contributions reconciling exactly with the portfolio return, explicit group totals, an explicitly declared benchmark (never auto-selected), Brinson-Fachler or Brinson-Hood-Beebower allocation/selection/interaction with residuals reported verbatim, arithmetic versus Carinó linking with the compounding gap disclosed, costs kept separate over a stated costed basis, and tracking error / information ratio / active drawdown. Measurements under a stated convention — never proof of alpha or manager skill, never a benchmark or portfolio recommendation." },
