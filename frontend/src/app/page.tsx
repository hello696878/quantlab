...
import PortfolioAttributionPanel from "@/components/PortfolioAttributionPanel";
...
const VIEW_META: Record<View, { title: string; subtitle: string }> = {
  ...
  portfoliostress: {
    title: "Portfolio Stress Testing, Scenario Shock & Drawdown Attribution Lab",
    subtitle:
      "A local-first portfolio stress lab: explicit deterministic scenarios — historical replays of actual stored observations (ex-ante only when the window ends strictly before the portfolio decision cutoff; full-sample windows stay descriptive), hypothetical asset / group / global shocks with unambiguous units (return decimal, percent, basis points — absolute price shocks are unsupported without reference prices) under a documented precedence and an explicit missing-shock policy, volatility and correlation stress (multiplicative / additive / toward-one / supplied) with PSD validation and a never-silent explicit repair policy, and liquidity / cost stress applied to copies of linked Phase 55 cost models with new fingerprints — applied to stored Phase 56 portfolio weights, with contribution attribution reconciled to the scenario total, post-shock drifted weights and constraint re-checks (no automatic rebalancing), baseline-vs-stressed marginal / component / percentage risk contributions, trailing-peak-only drawdown episodes with honest unrecovered states, per-asset interval attribution under a labelled static-weight approximation, and bounded one-at-a-time sensitivity probes — all in local SQLite with deterministic SHA-256 fingerprints. Scenarios are assumptions, not predictions; no scenario is a worst case; measured losses are not guarantees; nothing here hedges, rebalances, trades, recommends an action, proves safety or robustness, or constitutes investment, trading, or risk-management advice.",
  },
  portfolioattribution: {
    title: "Portfolio Performance Attribution, Benchmark & Active Risk Lab",
    subtitle:
      "A local-first performance-attribution lab: measured contributions of STORED portfolio weights — beginning-of-period weights known before the period they govern, drifting between rebalances by the same recursion the portfolio lab records, so a later rebalance or a later return can never change an earlier contribution — decomposed against an EXPLICITLY declared benchmark (fixed-weight, per-period or buy-and-hold; never auto-selected, never an implicit equal-weight fallback, and never renormalized silently) under a stated simple-return convention. Asset and group contributions reconcile exactly with the portfolio market return (groups come from stored labels, never inferred from asset names, and never overlap); single-period Brinson-Fachler or Brinson-Hood-Beebower allocation / selection / interaction effects reconcile with active return, with residuals reported verbatim and never redistributed, and one-sided or zero-weight groups leaving their terms honestly unavailable rather than fabricated. Multi-period effects are linked either arithmetically (with the compounding gap against the geometric active return disclosed) or by Carinó smoothing (reconciling with the compounded active return, using the exact equal-return limit); transaction costs from linked Phase 55 estimates stay strictly separate from market contribution over a stated costed basis; tracking error, information ratio (unavailable — never infinite — when tracking error is zero), hit rates and a relative active drawdown, contribution concentration, stored-regime views and stored drawdown-episode views complete the picture — all in local SQLite with deterministic SHA-256 fingerprints. Nothing here proves alpha or manager skill, recommends a benchmark or a portfolio, guarantees future performance, produces GIPS-compliant reporting, performs tax accounting, executes trades, or constitutes investment, trading, or performance-reporting advice.",
  },
  factordiagnostics: {
    title: "Factor Exposure, Return Decomposition & Macro Sensitivity Lab",
    subtitle:
      "A local-first factor research lab: measured sensitivities of ONE explicitly declared return series — a stored Phase 58 portfolio, benchmark, active or cost-adjusted return, or a supplied descriptive series — to SUPPLIED factor and macro observations. Every factor states its category, source unit, transformation formula (level, simple return, percent or log change, first difference, basis-point change from a declared rate unit, or a strictly trailing z-score that never reads its own or a later observation), integer lag and availability rule; alignment is by exact timestamp with nothing resampled, forward-filled, interpolated or zero-filled, and a period whose factor value is missing leaves the sample with a stated reason. Timing is explicit: lagged causal alignment is VERIFIED only when each value was knowable before the period it explains, contemporaneous alignment stays descriptive and is never called predictive, macro vintages can be selected first-release or as-of-cutoff so a later revision never reaches an earlier fit, and a future-looking alignment is accepted only when the caller declares it INVALID. Ordinary least squares is solved by SVD with honest rank, singular values and condition number; a rank-deficient design either fails or records a labelled minimum-norm solution with every standard error withheld; classical covariance is never called robust; ridge is an explicit reference with no p-values and no automatic lambda; R-squared is unavailable rather than 0 or 1 for a constant target; variance inflation is unavailable rather than infinite under exact collinearity. Contributions reconcile period by period against the estimator's own residual, trailing rolling windows never read an observation after their own end, held-out evaluation applies fixed training coefficients and benchmarks against the TRAINING mean, and stored regime, stress and attribution records are read-only. Nothing here proves causality, proves alpha, proves manager skill, predicts future returns, recommends a factor exposure, a macro trade or a portfolio, hedges, allocates, executes trades, certifies a factor model, or constitutes investment, trading, or risk-management advice. No market or macroeconomic data is ever downloaded.",
  },
  signaldecay: {
    title: "Signal Decay, Forecast Horizon, Turnover & Implementation Lag Lab",
    subtitle:
      "A local-first signal-decay lab: descriptive associations between STORED signal observations — scores, probabilities, ranks or indicators, with an explicitly declared direction that is never inferred from a name — and later outcomes across EXPLICIT forecast horizons and non-negative implementation delays on each entity's own stored grid. Signal availability timing is enforced (an outcome beginning before its signal was available marks the whole run invalid), forward returns come from explicit entry/exit prices with nothing interpolated or back-filled, horizon overlap is measured and disclosed with every classical p-value carrying that limitation, cross-sectional rank IC uses only each timestamp's own eligible universe, ties, constants and small samples are conservatively unavailable with reasons, equal-count buckets keep their boundaries visible with a neutral equal-weight top-minus-bottom reference whose gross and cost-adjusted results stay separate, one-way turnover and holding-cohort overlap are measured with declared initial and normalisation policies, decay summaries locate sign changes without calling any horizon optimal, half-life exists only when an exponential fit is mathematically coherent, and stored regime, validation, cost, meta-labeling, feature and factor records are read-only and fingerprint-pinned. Nothing here proves predictability or alpha, guarantees that a relationship persists, recommends a horizon, lag, threshold, signal or trade, selects a strategy, sizes a position, monitors or executes anything, or constitutes investment, trading or risk-management advice. No market or alternative data is ever downloaded.",
  },
  signalensemble: {
    title: "Signal Ensemble, Redundancy & Combination Lab",
    subtitle:
...
export default function HomePage() {
  ...
  // Navigation command data lives in the canonical workspace registry so the
  // palette, the sidebar and the drift guards read one list (Phase 63.0).
  const NAV_COMMANDS = WORKSPACE_COMMANDS;
  ...
  return (
    ...
    >
      <div className="space-y-8">
        ...
        {view === "portfoliostress" && <PortfolioStressPanel onNav={(route) => handleNav(route as View)} />}
        {view === "portfolioattribution" && <PortfolioAttributionPanel onNav={(route) => handleNav(route as View)} />}
